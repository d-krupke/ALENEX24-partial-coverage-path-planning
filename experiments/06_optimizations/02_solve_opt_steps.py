...
def solve(d):
    ...
    cb = Callbacks()
    solvers += [
        MeshAlgorithm(full_coverage=False, cc_opt_steps=k, t_opt_steps=0, callbacks=cb)
        for k in [0, 10, 25, 50, 100, 200]
    ]
    solvers += [
        MeshAlgorithm(full_coverage=False, cc_opt_steps=0, t_opt_steps=k, callbacks=cb)
        for k in [0, 10, 25, 50, 100, 200]
    ]
