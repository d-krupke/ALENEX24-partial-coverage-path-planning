...
from aemeasure.context_printer import print_context
...
os.makedirs("./solutions_10/", exist_ok=True)
...
def solve(d):
    ...
    solvers += [
        MeshAlgorithm(
            full_coverage=False, cc_opt_steps=0, t_opt_steps=0, opt_size=0, callbacks=cb
        )
    ]
    solvers += [
        MeshAlgorithm(
    ...
    solution_path = os.path.join("./solutions_10/", f"{instance_name}.results.json")
    ...
    for i, solver in enumerate(solvers):
        ...
        with Measurement(solution_path) as m:
            ...
            m["opt_area"] = solver.grid_solver.params.t_opt_size
