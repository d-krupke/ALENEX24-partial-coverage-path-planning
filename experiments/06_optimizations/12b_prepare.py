...
tables = []
for f in os.listdir("./solutions_12"):
    if not f.endswith(".results.json"):
        continue
    f = os.path.join("./solutions_12", f)
...
data["gobj_change"] = 100 * (data["grid_obj_tour"] / data["gobj_wo"] - 1)
data["gobj_opt_gap"] = 100 * (data["grid_obj_tour"] / data["grid_lb"] - 1)

data.to_json("12c_prepared_data.json")
